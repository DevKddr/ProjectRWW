...
public:
	...
	// PlayerData.db를 열고, PlayerData 테이블이 없으면 생성한다.
...
private:
	// TUniquePtr 대신 TPimplPtr을 쓴다 — 전방 선언된 타입(FSQLiteDatabase)을 멤버로 가질 때
	// 생성자/소멸자를 따로 선언하지 않아도 되도록 UE가 제공하는 전용 스마트 포인터.
